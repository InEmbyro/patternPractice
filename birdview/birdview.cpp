...
unsigned long rcvIdent[] = {
	...
	0x620, 0x621, 0x622, 0x623, 0x624, 0x625, 0x626, 0x627, 0x628, 0x629, 0x62a, 0x62b, 0x62c, 0x62d, 0x62e, 0x62f, 0x00};
...
#define HALF_CAR_HEIGHT	(0.25f)
	...
	glNormal3f(0.0f, 0.0f, -1.0f);
	glBegin(GL_QUADS);
	glVertex3f(tempX - halfCarWidth, tempY - HALF_CAR_HEIGHT, tempZ + halfCarLen);
	glVertex3f(tempX - halfCarWidth, tempY + HALF_CAR_HEIGHT, tempZ + halfCarLen);
	glVertex3f(tempX + halfCarWidth, tempY + HALF_CAR_HEIGHT, tempZ + halfCarLen);
	glVertex3f(tempX + halfCarWidth, tempY - HALF_CAR_HEIGHT, tempZ + halfCarLen);
	glEnd();
	...
	glPushMatrix();
	glColor3f(0.8f, 0.8f, 0.8f);
	glRasterPos3f(tempX, tempY + HALF_CAR_HEIGHT, tempZ);
	sprintf(quote, "%d", pRaw->TargetNum);
	glCallLists(strlen(quote), GL_UNSIGNED_BYTE, quote);
	glPopMatrix();
...
BOOL CBirdviewView::ParseRawObject2nd(PARAM_STRUCT *pSrc, RAW_OBJECT_STRUCT *pRaw)
{
	...
	pRaw->x_range = pRaw->range * cos(pRaw->theta * RAD_CONVER) * cos(pRaw->angle * RAD_CONVER);
	pRaw->y_range = pRaw->range * cos(pRaw->theta * RAD_CONVER) * sin(pRaw->angle * RAD_CONVER);
...
//	glRotatef(SENSOR_ROTATE, 0.0f, 1.0f, 0.0f);
	...
	glBegin(GL_QUADS);
	glNormal3f(0.0f, 0.0f, -1.0f);
#define halfCanHeight	1.0f
	glVertex3f(-halfCarWidth, halfCanHeight, halfCarLen);
